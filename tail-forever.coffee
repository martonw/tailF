...
class Tail extends events.EventEmitter
  ...
  constructor:(@filename,  @options = {}) ->
    ...
    assert.ok us.isFunction(@options.syncCallback), "syncCallback should be function" if @options.syncCallback?

    @separator = @options?.separator? || '\n'
    @buffer = ''
    @queue = new SeriesQueue(@_readBlock, @options.syncCallback)
    @isWatching = false
    @bookmarks = {}
